...
//if I don't comment this, a error will occur    
 //   if (nw) {
	//for (int w = 0; w < V; w++) {
	//    if (nw[w]) {
	//	delete nw[w];
...
int model::init_est() {
	...
	/*for all documents m [1; M] do
		for all words n [1; Nm] in document m do
			...
			increment document每topic count: n(k)
			m + 1
			increment document每topic sum: nm + 1
			increment topic每term count: n(t)
			k + 1
			increment topic每term sum: nk + 1
		end for
	end for*/
...
void model::estimate() {
	...
	// for all z_i
	//PLEASE﹛LOOK THE PARAMETER ESTIMATION FOR TEXTANALYSIS.PDF FOR FUTHER UNDERSTANGING
	...
	//if converged and L sampling iterations since last read out then
		//	the dierent parameters read outs are averaged.
		//	read out parameter set  according to Eq. 82
		//	read out parameter set  according to Eq. 83
	//end if
...
//m is the number of the document, n is the number of the word(mention: not index)
int model::sampling(int m, int n) {
    // remove z_i from the count variables
    int topic = z[m][n];
	// n is the number of the word, however, w is the index in the wordmap
    int w = ptrndata->docs[m]->words[n];
    nw[w][topic] -= 1;
    nd[m][topic] -= 1;
    nwsum[topic] -= 1;
